# MD Card Mod Tool 2.0.15

手机资源现在直接进入原主工作区：点击“选择目录”选已解压的 0000（例如 D:\qq\0000.zip\0000），使用与 PC 相同的卡名搜索、替换所选、制作超框、视觉资源、怪兽动画和 Mod 管理。不存在单独的手机精简窗口。首次扫描实际手机 Bundle，备份位于所选 0000 下的 _MD卡图备份；旧 2.0.14 的 _MDMobileMods 备份不自动迁移。

手机端按实际完整动画档位处理（22524 为三 Bundle），生成使用 RGBA32，可能增加文件／内存占用。直接替换 ZIP 输出 0000/哈希目录，复制回手机只需要实际哈希文件，不要复制工具备份／草稿。已通过临时手机文件的共享流程测试，未验收全量资源或手机游戏内效果。本地测试版尚未上传 GitHub。
...
下载 [2.0.12 发行版](https://github.com/noah-ad/MD-Card-Mod-Tool/releases/tag/v2.0.12)。解压后根目录只有 `MD卡图查看替换器.exe` 与 `data` 文件夹，直接运行 EXE 即可；若文件带有 Windows 下载标记，可先运行 `data\首次启动-解除下载封锁.bat`。
